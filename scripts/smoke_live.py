...
def main(base: str) -> int:
    ...
    # per-vendor buckets, so Claude-User / Claude-SearchBot (user-triggered
    # and search) are named separately from ClaudeBot (training). Those three
    # lines are ARTIFACT evidence and stay pinned to Allow; the training
    # crawlers moved to the posture check below.
    ...
    # stanza at all — absent or Allow are both the allow shape, so assert the
    # absence of Disallow rather than the presence of Allow. A host whose
