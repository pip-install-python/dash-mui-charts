...
def parse_changelog(path: Path = CHANGELOG_PATH) -> list[dict]:
    ...
    for line in path.read_text(encoding="utf-8").split("\n"):
        ...
        # A RELEASE heading, not merely an `##` heading (fork finding,
        # 2026-08-31). 1.6.41 widened this to accept bare versions for
        # pannellum's `## 2.0.0 — date`, and the widening swallowed prose:
        # this fork's CHANGELOG.md ends with `## Component License
        # Requirements`, which parsed as a release, rendered a card badged
        # with that sentence, and pushed the count to "15 releases" for 14.
        # A release label is bracketed, or a version, or a date, or
        # Unreleased — a free-text `##` section is none of those and belongs
        # to whatever release it sits under.
        if vm and not (line.startswith("## [")
                       or _is_version(vm.group("label").strip())
                       or re.fullmatch(r"\d{4}-\d{2}-\d{2}", vm.group("label").strip())
                       or vm.group("label").strip().lower() == "unreleased"):
            vm = None
        if vm:
            close_section()
