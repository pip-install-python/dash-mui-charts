...
Fork seam: this page hides itself from every machine surface at its own
import (``mark_hidden`` below), the way ``pages/control_board.py`` does.
Since sync item 16 the sidebar and the header search both exclude
``/admin/*`` structurally (``components.navbar.is_nav_page``), so the
hand-kept EXCLUDED_LINKS set this fork carried is gone — the two halves
that used to be kept in step are one rule now, pinned by
``tests/test_nav_contract.py`` and ``tests/test_excluded_links_hidden.py``.
"""
