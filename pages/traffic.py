...
Fork seam (DIVERGENCES §5): upstream the navbar marks every excluded path
hidden at import; here each admin page hides itself at its own import (the
``mark_hidden`` below) and ``components.navbar.EXCLUDED_LINKS`` keeps it out
of the sidebar and the header search. Both halves are required —
``tests/test_control_board.py::test_every_excluded_link_is_machine_hidden``
pins the set, not the single path, for exactly this second entry.
"""
