...
def test_the_markdown_page_builder_concatenates_rather_than_nesting():
    ...
    THIS FORK'S composition sites, which are not the template's:
    pages/markdown.py concatenates (`section + layout`) and is the only
    place a parser result is composed. pages/home.py is hand-written Dash
    with no parser at all (DIVERGENCES §8), so it carries no `create_parser`
    call to pin — checking it here would pin the absence of a call that
    must not exist. lib/directives/ is where a directive could compose one,
    so it is swept instead.
    """
    for fname, parser_name in (("pages/markdown.py", "parse"),
                               ("lib/directives/kwargs.py", "props_for"),
                               ("lib/directives/source.py", "parse")):
        path = REPO_ROOT / fname
        if not path.exists():                      # a directive this fork lacks
            continue
        src = path.read_text()
        tree = ast.parse(src)
        nested = []
