...
Adapted from the template at 1.6.29 (5589318) per SYNC-1.6.22-1.6.29 item 6,
which classes this file fork-OWNED and never block cargo. This fork had no
copy at all until 2026-08-26, and that absence is exactly what the item
describes: the 1.6.28 fan-out byte-copied the script in, the suite went green
because nothing here exercised it, and the new crawler/browser identity check
found a real defect on the wire instead — CD run 33002187234, red at
"Smoke-test the deployment". With this file present the same defect fails
