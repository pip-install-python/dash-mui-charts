...
DEMOS: dict[str, dict] = {
    # This site's hero: gate /pie (control board or `tier: auth`
    # frontmatter) and its sign-in card renders the interactive PieChart
    # above the "Authentication required" copy — click a slice, watch the
    # callback answer, then sign in for the rest.
    #
    # Chosen deliberately over the flashier Pro pages: the teaser must
    # render the SAME for everyone, and /linechart-pro, /crosshair and the
    # other 15 Pro demos degrade to a license banner without
    # MUI_PRO_API_KEY. PieChart is license-free, self-contained, and sized
    # (height=300) to sit inside the card without scrolling.
    "/pie": {
        "module": "docs.pie.interactive_example",
        "caption": "Click a slice — live PieChart",
        "max_height": 420,
    },
}
