...
# verbatim, and an unknown key is a TypeError at boot (measured on the
# template, sync item 16 note (a)).
ICONS: Dict[str, str] = {
...
def other_apps_for(app_url: str) -> List[Dict[str, str]]:
    ...
    PEERS + AFFILIATED, in registry order, this app removed, each labelled by
    its DOMAIN — short enough for a menu, and the one name every host agrees
    on. Never hand-typed twice again: the survey found 2plot.dev listed twice
    on four hosts and a retired domain on one. `icon` comes from ICONS above
    (default `mdi:web`)."""
    own = app_url.rstrip("/")
    out = []
