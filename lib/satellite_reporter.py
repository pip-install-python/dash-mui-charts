...
Env:
    ...
    SATELLITE_APP_KEY           network directory key for this app (default
                                "muicharts", this app's own directory key)
...
def app_key() -> str:
    ...
    Default is "muicharts" (this app's directory key on every hub surface,
    STANDARD §5) — identity-adapted from the boilerplate's "boilerplate"
    fallback so that a deployment missing SATELLITE_APP_KEY can only ever
    report as ITSELF, never under another app's row. The CODE above this
    line stays byte-identical to the template's; only the identity moves.
    """
    return os.getenv("SATELLITE_APP_KEY") or "muicharts"
